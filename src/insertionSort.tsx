...
import { insertionSort, SortResult, Step } from "./insertionSort";
...
const InsertionSortVisualization: React.FC = () => {
  ...
  const getElementClass = (index: number) => {
    ...
    if (index === step.comparingIndex) return "bg-blue-300";
    if (index < step.currentIndex) return "bg-green-200";
    return "bg-white";
  };
        ...
        {sortResult &&
          ...
          currentStep < sortResult.steps.length && (
            <div className="text-center text-xs text-green-400">
              <p>Current Index: {sortResult.steps[currentStep].currentIndex}</p>
              <p>
                Comparing Index: {sortResult.steps[currentStep].comparingIndex}
              </p>
              <p>
                Inserted:{" "}
                {sortResult.steps[currentStep].inserted ? "Yes" : "No"}
              </p>
            </div>
          )}
        {isSorted && (
